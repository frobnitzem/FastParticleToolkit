...
namespace alpaka
{
    namespace test
    {
        ...
        template<typename TAcc, typename QueueAcc>
        ...
        public:
            ...
            KernelExecutionFixture(TExtent const& extent, QueueAcc &queue)
            ...
            KernelExecutionFixture(WorkDiv const& workDiv, QueueAcc &queue)
                : m_devHost(alpaka::getDevByIdx<PltfCpu>(0u))
                , m_devAcc(alpaka::getDevByIdx<PltfAcc>(0u))
                , m_queue(queue)
                , m_workDiv(workDiv)
